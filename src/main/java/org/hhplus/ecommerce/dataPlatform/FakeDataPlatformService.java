...
public class FakeDataPlatformService implements DataPlatformService {
    ...
    @Override
    public void sendData(OrderPaySuccessEvent event) {

        List<Long> orderItemIds = event.getOrderItemInfos().stream()
        ...
        try {
            ...
            slackWebhook.send("데이터 플랫폼 전송에 실패했습니다. orderItemIds: " + orderItemIds);
